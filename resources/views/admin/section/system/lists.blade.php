			...
			<div class="ddrtabs__content ddrtabscontent" ddrtabscontent>
				...
				<div class="ddrtabscontent__item" ddrtabscontentitem="sectionsTab4">
					<x-card
						...
						<x-simplelist
							...
							fieldset="ID:w7rem|number|id|1,Название типа заказа:w30rem|text|title,Короткое название:w14rem|text|title_short,Цвет:w6rem|color|color,Совпадения:w30rem|textarea|matches,Исключения:w30rem|textarea|exceptions,Сорт:w7rem|number|sort,Отобр в стат:w7rem|checkbox|show_in_stat"
							{{-- options="name_type;foo:fooval,bar:barval|name_radio;foo:rool,bar:tool" --}}
							{{-- onRemove="removeCustomerAction" --}}
							group="normal"
						 />
				...
				<div class="ddrtabscontent__item" ddrtabscontentitem="sectionsTab6">
					...
					<code class="d-block mb2rem">
						...
						<p class="color-gray-500 color-gray-600-hovered pointer color-green-active" onclick="$.copyToClipboard(event, '@{{date_timesheet}}')" title="Скопировать">@{{date_timesheet}} 		- Дата события</p>
					</code>
					
					
					<x-simplelist
						setting="slack_notifies"
						fieldset="ID:w6rem|number|id|1,Название уведомеления:w20rem|text|title,Цвет кнопки:w20rem|select|color,Иконка:w15rem|text|icon,Тайаут (сек):w7rem|number|timeout,Вебхук:w30rem|textarea|webhook,Сообщение:w30rem|textarea|message,Право:w15rem|text|permission,Логировать:w7rem|checkbox|logging"
