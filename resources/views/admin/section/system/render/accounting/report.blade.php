				...
				<x-table.tr class="h3rem">
					...
					@foreach($allPeriods as $pId => $pTitle)
						<x-table.td class="h-end">
							@isset($report[$cId][$pId])
								<p>@number($report[$cId][$pId]) @symbal(dollar)</p>
							@else
					...
					@endforeach
					<x-table.td class="w-auto"></x-table.td>
					<x-table.td class="h-end"><strong>@number($report[$cId]['all'] ?? '-') @symbal(dollar)</strong></x-table.td>
				</x-table.tr>
			@endforeach
		</x-table.body>
		<x-table.foot>
			<x-table.tr class="h5rem">
				...
				@foreach($allPeriods as $pId => $pTitle)
					<x-table.td class="w10rem h-end">
						@isset($report['periods'][$pId])
							<strong>@number($report['periods'][$pId]) @symbal(dollar)</strong>
						@else
							<p class="color-gray-400">-</p>
						@endisset
					</x-table.td>
				@endforeach
				<x-table.td class="w-auto"></x-table.td>
				<x-table.td class="w15rem h-end"><strong>@number($report['total']) @symbal(dollar)</strong></x-table.td>
			</x-table.tr>
